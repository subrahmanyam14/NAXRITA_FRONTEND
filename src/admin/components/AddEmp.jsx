...
const TOAST_ID = 'add-employee-toaster';
...
const AddEmployee = () => {
  ...
  const fetchRoles = async () => {
    ...
    try {
      ...
      toast.error('Error fetching roles. Please try again.', { containerId: TOAST_ID });
      setRoles([]);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!employeeData || !jobDetailsData) {
      toast.error('Form data is not properly initialized', { containerId: TOAST_ID });
      return;
    }
    if (!employeeData.employee_id?.trim()) {
      toast.error('Employee ID is required', { containerId: TOAST_ID });
      return;
    }
    if (!employeeData.employee_name?.trim()) {
      toast.error('Employee name is required', { containerId: TOAST_ID });
      return;
    }
    if (!isValidEmail(employeeData.email)) {
      toast.error('Please enter a valid email address', { containerId: TOAST_ID });
      return;
    }
    if (!employeeData.joining_date || !employeeData.hire_date) {
      toast.error('Joining date and hire date are required', { containerId: TOAST_ID });
      return;
    }
    if (
      isNaN(new Date(employeeData.joining_date).getTime()) ||
      isNaN(new Date(employeeData.hire_date).getTime())
    ) {
      toast.error('Invalid date format', { containerId: TOAST_ID });
      return;
    }
    if (!employeeData.department_name?.trim()) {
      toast.error('Department name is required', { containerId: TOAST_ID });
      return;
    }
    if (!employeeData.job_profile_progression_model_designation?.trim()) {
      toast.error('Designation is required', { containerId: TOAST_ID });
      return;
    }
    if (!jobDetailsData.phone?.trim()) {
      toast.error('Phone number is required', { containerId: TOAST_ID });
      return;
    }
    if (!isValidPhoneNumber(jobDetailsData.phone)) {
      toast.error('Please enter a valid phone number', { containerId: TOAST_ID });
      return;
    }
    ...
    try {
      ...
      const jobDetailsPayload = {
        ...
        skills: (jobDetailsData.skills || []).filter((s) => s?.trim() !== '')
      };
      ...
      toast.success('Employee added successfully!', { containerId: TOAST_ID });
      closeForm();
    } catch (error) {
      ...
      if (error.response?.status === 409) {
        toast.error('Employee ID already exists. Please use a different ID.', { containerId: TOAST_ID });
      } else if (error.response?.data?.errors) {
        toast.error(error.response.data.errors.join(', '), { containerId: TOAST_ID });
      } else {
        toast.error(error.message || 'Error adding employee. Please try again.', { containerId: TOAST_ID });
      }
    }
  };
    ...
    <>
      ...
      {createPortal(
        <ToastContainer
          ...
          style={{ top: 140, right: '10%', zIndex: 1000000 }}
          newestOnTop
          closeOnClick
        />,
        document.body
      ...
      <style jsx>{`
        ...
        .gradient-border { background: linear-gradient(135deg, #1e40af, #3b82f6, #60a5fa); padding: 1px; border-radius: 12px; }
        .gradient-border-content { background: #0a0a0a; border-radius: 11px; height: 100%; width: 100%; }
        .glow-button { background: linear-gradient(135deg, #1e40af, #3b82f6); box-shadow: 0 0 20px rgba(59, 130, 246, 0.3); transition: all 0.3s ease; }
        .glow-button:hover { box-shadow: 0 0 30px rgba(59, 130, 246, 0.5); transform: translateY(-1px); }
        ...
        {isOpen &&
          createPortal(
            <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-50 p-4">
              <div
                className={`relative bg-black border border-gray-800 rounded-2xl w-full max-w-7xl max-h[95vh] max-h-[95vh] transition-all duration-300 shadow-2xl ${
              ...
              >
                ...
                <div className="flex justify-between items-center p-6 border-b border-gray-800">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-2xl font-bold gradient-text">Create New Employee</h3>
                  </div>
                  <button
                ...
                <div className="overflow-y-auto noscrollbar max-h-[calc(95vh-160px)]">
                  <form onSubmit={handleSubmit} className="p-6">
                    ...
                    <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-gray-800">
                      ...
                      <button type="submit" className="glow-button px-6 py-2 text-white font-medium rounded-lg text-sm">
                        <span className="gradient-text">Create Employee</span>
                      </button>
                    </div>
                  </form>
