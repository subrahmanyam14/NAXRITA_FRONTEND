...
const api = {
  // Employee APIs
  getEmployee: async () => {
    const response = await fetch(`${process.env.REACT_APP_URL}/api/employees/employee`, {
      headers: {
  ...
  getPersonalDetails: async () => {
    const response = await fetch(`${process.env.REACT_APP_URL}/api/personalDetails/individual`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`,
        'Content-Type': 'application/json'
      }
  ...
  updatePersonalDetails: async (id, data) => {
    ...
    const response = await fetch(`${process.env.REACT_APP_URL}/api/personalDetails/${id}`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      },
  ...
  // Job Details APIs
  getJobDetails: async () => {
    const response = await fetch(`${process.env.REACT_APP_URL}/api/jobDetails/employee`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`,
        'Content-Type': 'application/json'
      }
...
// Starfield Background Component (kept but unused)
...
const PasswordUpdateModal = ({ isOpen, onClose, onUpdate }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-[#1e1e1e] rounded-xl border border-[#2a2a2a] p-4 md:p-6 w-[92%] max-w-md mx-4 animate-in fade-in zoom-in duration-300">
        <div className="absolute inset-0 bg-gradient-to-br from-[#2563eb]/10 to-transparent rounded-xl" />
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-4 md:mb-6">
            <h3 className="text-lg md:text-xl font-light text-white flex items-center space-x-3">
          ...
          <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
            <div>
              <label className="block text-xs md:text-sm font-light text-[#a3a3a3] mb-1.5 md:mb-2">Current Password</label>
              <div className="relative">
                <input
                  type={showPasswords.current ? 'text' : 'password'}
                  value={passwordData.currentPassword}
                  onChange={(e) => setPasswordData(prev => ({ ...prev, currentPassword: e.target.value }))}
                  className="w-full px-3 md:px-4 py-2.5 md:py-3 pr-10 md:pr-12 bg-[#161616] border border-[#404040] rounded-lg focus:ring-2 focus:ring-[#2563eb] focus:border-transparent text-white font-light text-sm md:text-base"
                  required
                />
                <button
                  type="button"
            ...
            <div>
              <label className="block text-xs md:text-sm font-light text-[#a3a3a3] mb-1.5 md:mb-2">New Password</label>
              <div className="relative">
                <input
            ...
            <div>
              <label className="block text-xs md:text-sm font-light text-[#a3a3a3] mb-1.5 md:mb-2">Confirm New Password</label>
              <div className="relative">
                <input
            ...
            <div className="flex space-x-2 md:space-x-3 pt-2 md:pt-4">
              <button
                type="button"
                onClick={onClose}
                className="flex-1 px-3 md:px-4 py-2.5 text-xs md:text-sm font-light text-[#a3a3a3] bg-transparent border border-[#404040] rounded-lg hover:text-white hover:border-white transition-all"
              >
                Cancel
              </button>
              <button
                type="submit"
                disabled={loading}
                className="flex-1 px-3 md:px-4 py-2.5 text-xs md:text-sm font-light text-white bg-[#2563eb] rounded-lg hover:bg-[#1d4ed8] transition-all hover:shadow-[0_0_20px_rgba(37,99,235,0.4)] disabled:opacity-50 flex items-center justify-center space-x-2"
              >
                {loading ? (
                  <>
...
const ConfirmationModal = ({ isOpen, onClose, onConfirm, title, message, type = 'warning' }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={onClose} />
      <div className={`relative bg-[#1e1e1e] rounded-xl border p-4 md:p-6 w-[92%] max-w-md mx-4 animate-in fade-in zoom-in duration-300 ${typeStyles[type]}`}>
        <div className="text-center">
          <h3 className="text-base md:text-lg font-light text-white mb-2 md:mb-3">{title}</h3>
          <p className="text-xs md:text-sm font-light text-[#a3a3a3] mb-4 md:mb-6">{message}</p>
          <div className="flex space-x-2 md:space-x-3">
            <button
              onClick={onClose}
              className="flex-1 px-3 md:px-4 py-2 text-xs md:text-sm font-light text-[#a3a3a3] bg-transparent border border-[#404040] rounded-lg hover:text-white hover:border-white transition-all"
            >
              Cancel
            </button>
            <button
              onClick={onConfirm}
              className="flex-1 px-3 md:px-4 py-2 text-xs md:text-sm font-light text-white bg-[#2563eb] rounded-lg hover:bg-[#1d4ed8] transition-all"
            >
              Confirm
            </button>
          </div>
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center relative overflow-hidden font-jakarta">
        <div className="text-center relative z-10">
          <div className="w-12 h-12 md:w-16 md:h-16 border-4 border-[#2563eb]/30 border-t-[#2563eb] rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-sm md:text-lg font-light text-[#a3a3a3]">Loading your cosmic profile...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <style>{`
        ...
        .cosmic-glow { box-shadow: 0 0 20px rgba(59, 130, 246, 0.3); }
        .hover-lift { transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); }
        .hover-lift:hover { transform: translateY(-4px); box-shadow: 0 10px 25px rgba(0, 0, 0, 0.9), 0 0 20px rgba(59, 130, 246, 0.2); }
        ...
        /* Hide scrollbar for the tabs on mobile for cleaner look */
      ...
      <div className="transition-all duration-300">
                ...
                {editMode ? (
                  <>
                    <button
                      ...
                      className="px-3 md:px-4 py-2 text-xs md:text-sm font-light text-[#a3a3a3] bg-transparent border border-[#404040] rounded-lg hover:text-[#ffffff] hover:border-[#ffffff] transition-all"
                    >
                    ...
                    <button
                      ...
                      disabled={saving}
                      className="px-3 md:px-4 py-2 text-xs md:text-sm font-light text-white bg-[#2563eb] rounded-lg hover:bg-[#1d4ed8] transition-all hover:shadow-[0_0_20px_rgba(37,99,235,0.4)] disabled:opacity-50 flex items-center space-x-2"
                    >
                      {saving ? (
                        <>
                  ...
                  <button
                    ...
                    className="px-3 md:px-4 py-2 text-xs md:text-sm font-light text-white bg-[#2563eb] rounded-lg hover:bg-[#1d4ed8] transition-all hover:shadow-[0_0_20px_rgba(37,99,235,0.4)] flex items-center space-x-2"
                  >
        ...
        <div className="p-4 md:p-8 space-y-4 md:space-y-6 relative z-10">
         ...
         <div
              ...
              <div className="flex items-center space-x-4 md:space-x-6">
                <div className="relative group">
                  <img
                    ...
                    className="w-20 h-20 md:w-24 md:h-24 rounded-xl object-cover border-2 border-[#2563eb]/50 transition-all duration-300 group-hover:border-[#2563eb]"
                  />
                  {editMode && (
                    <label className="absolute -bottom-2 -right-2 w-7 h-7 md:w-8 md:h-8 bg-[#2563eb] text-white rounded-full flex items-center justify-center hover:bg-[#1d4ed8] transition-colors shadow-lg cursor-pointer">
                  ...
                  <div className="absolute -top-1 -right-1 w-5 h-5 md:w-6 md:h-6 bg-[#22c55e] rounded-full border-2 border-[#1e1e1e] flex items-center justify-center">
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between">
                    <div className="min-w-0">
                      <h2 className="text-lg md:text-2xl font-light text-[#ffffff] mb-0.5 md:mb-1 truncate">
                        {personalData?.gender ? `${personalData.gender} Employee` : employeeData?.employee_name || 'Employee Name'}
                      </h2>
                      <p className="text-sm md:text-lg font-light text-[#a3a3a3] mb-1 md:mb-2 truncate">
                      ...
                      <div className="flex flex-wrap items-center gap-2 md:space-x-4">
                        <span className="flex items-center space-x-1.5 md:space-x-2 text-xs md:text-sm font-light text-[#6b7280]">
                          <Building2 className="h-3.5 w-3.5 md:h-4 md:w-4 text-[#2563eb]" />
                          <span className="truncate">{employeeData?.department_name || 'Department'}</span>
                        </span>
                        <span className="flex items-center space-x-1.5 md:space-x-2 text-xs md:text-sm font-light text-[#6b7280]">
                          <Calendar className="h-3.5 w-3.5 md:h-4 md:w-4 text-[#2563eb]" />
                          <span>Since {employeeData?.joining_date ? new Date(employeeData.joining_date).getFullYear() : 'N/A'}</span>
                        </span>
                        <span className="flex items-center space-x-1.5 md:space-x-2 text-xs md:text-sm font-light text-[#6b7280]">
                          <MapPin className="h-3.5 w-3.5 md:h-4 md:w-4 text-[#2563eb]" />
                          <span className="truncate">{jobData?.location || personalData?.address || 'Location'}</span>
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-1.5 md:space-x-3 ml-2">
                      ...
                      <span className="px-2 md:px-3 py-0.5 md:py-1 bg-[#2563eb]/20 text-[#2563eb] text-xs md:text-sm font-light rounded-full border border-[#2563eb]/30">
          ...
          <div className="bg-[#1e1e1e] rounded-xl border border-[#2a2a2a] p-1.5 md:p-2 backdrop-blur-lg">
            <div className="flex space-x-2 tabs-scroll overflow-x-auto">
              {[
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 md:gap-6">
            ...
            <div className="lg:col-span-3 space-y-4 md:space-y-6">
              {activeSection === 'personal' && (
                <div className="bg-[#1e1e1e] rounded-xl border border-[#2a2a2a] p-4 md:p-6 hover-lift backdrop-blur-lg">
                  <div className="absolute inset-0 bg-gradient-to-br from-[#2563eb]/5 to-transparent rounded-xl"></div>
                  <div className="relative z-10">
                    <h3 className="text-lg md:text-xl font-light text-[#ffffff] mb-4 md:mb-6 flex items-center space-x-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                      <div>
                        ...
                        {editMode ? (
                          <select
                            ...
                            className="w-full px-3 md:px-4 py-2.5 md:py-3 bg-[#161616] border border-[#404040] rounded-lg focus:ring-2 focus:ring-[#2563eb] focus:border-transparent text-[#ffffff] font-light text-sm md:text-base"
                          >
                      ...
                      <div>
                        ...
                        {editMode ? (
                          ...
                          <div className="flex items-center space-x-2.5 md:space-x-3 bg-[#111111] px-3 md:px-4 py-2.5 md:py-3 rounded-lg border border-[#2a2a2a]">
                            ...
                            <span className="text-sm md:text-lg font-light text-[#ffffff] truncate">
                      ...
                      <div>
                        ...
                        {editMode ? (
                          ...
                          <div className="flex items-center space-x-2.5 md:space-x-3 bg-[#111111] px-3 md:px-4 py-2.5 md:py-3 rounded-lg border border-[#2a2a2a]">
                            ...
                            <span className="text-sm md:text-lg font-light text-[#ffffff]">
                      ...
                      <div>
                        <label className="block text-xs md:text-sm font-light text-[#a3a3a3] mb-1.5 md:mb-2">Nationality</label>
                        {editMode ? (
                          <input
                            type="text"
                      ...
                      <div>
                        <label className="block text-xs md:text-sm font-light text-[#a3a3a3] mb-1.5 md:mb-2">Marital Status</label>
                        {editMode ? (
                          <select
                      ...
                      <div>
                        <label className="block text-xs md:text-sm font-light text-[#a3a3a3] mb-1.5 md:mb-2">Country of birth</label>
                        {editMode ? (
                          <input
                            type="text"
                            value={editPersonalData.country_birth || ''}
                            onChange={(e) => updatePersonalField('country_birth', e.target.value)}
                            className="w-full px-3 md:px-4 py-2.5 md:py-3 bg-[#161616] border border-[#404040] rounded-lg focus:ring-2 focus:ring-[#2563eb] focus:border-transparent text-[#ffffff] font-light text-sm md:text-base"
                          />
                        ) : (
                          <div className="flex items-center space-x-3 bg-[#111111] px-3 md:px-4 py-2.5 md:py-3 rounded-lg border border-[#2a2a2a]">
                            <span className="text-sm md:text-lg font-light text-[#ffffff]">
                              {personalData?.country_birth || 'Not specified'}
                            </span>
                          </div>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {editMode ? (
                          <select
                            value={editPersonalData.citizenship_status || ''}
                            onChange={(e) => updatePersonalField('citizenship_status', e.target.value)}
                            className="w-full px-3 md:px-4 py-2.5 md:py-3 bg-[#161616] border border-[#404040] rounded-lg focus:ring-2 focus:ring-[#2563eb] focus:border-transparent text-[#ffffff] font-light text-sm md:text-base"
                          >
                            <option value="">Select Status</option>
                          ...
                          <p className="text-sm md:text-lg font-light text-[#ffffff] bg-[#111111] px-3 md:px-4 py-2.5 md:py-3 rounded-lg border border-[#2a2a2a]">
                            {personalData?.citizenship_status || 'Not specified'}
                          </p>
                        )}
                      </div>
                      ...
                      <div className="md:col-span-2">
                        ...
                        {editMode ? (
                          <textarea
                            ...
                            rows={3}
                            className="w-full px-3 md:px-4 py-2.5 md:py-3 bg-[#161616] border border-[#404040] rounded-lg focus:ring-2 focus:ring-[#2563eb] focus:border-transparent text-[#ffffff] font-light text-sm md:text-base resize-none"
                          />
                        ) : (
                          <p className="text-sm md:text-lg font-light text-[#ffffff] bg-[#111111] px-3 md:px-4 py-2.5 md:py-3 rounded-lg border border-[#2a2a2a] leading-relaxed">
                      ...
                      <div>
                        <label className="block text-xs md:text-sm font-light text-[#a3a3a3] mb-1.5 md:mb-2">PAN ID</label>
                        {editMode ? (
                          <input
                            type="text"
                      ...
                      <div>
                        <label className="block text-xs md:text-sm font-light text-[#a3a3a3] mb-1.5 md:mb-2">Aadhar Number</label>
                        {editMode ? (
                          <input
                            type="text"
                      ...
                      <div>
                        <label className="block text-xs md:text-sm font-light text-[#a3a3a3] mb-1.5 md:mb-2">Emergency Contact Name</label>
                        {editMode ? (
                          <input
                            type="text"
                            value={editPersonalData.emergency_contact_name || ''}
                            onChange={(e) => updatePersonalField('emergency_contact_name', e.target.value)}
                            className="w-full px-3 md:px-4 py-2.5 md:py-3 bg-[#161616] border border-[#404040] rounded-lg focus:ring-2 focus:ring-[#2563eb] focus:border-transparent text-[#ffffff] font-light text-sm md:text-base"
                          />
                        ) : (
                          <p className="text-sm md:text-lg font-light text-[#ffffff] bg-[#111111] px-3 md:px-4 py-2.5 md:py-3 rounded-lg border border-[#2a2a2a]">
                            {personalData?.emergency_contact_name || 'Not specified'}
                          </p>
                        )}
                      </div>

                      <div>
                        <label className="block text-xs md:text-sm font-light text-[#a3a3a3] mb-1.5 md:mb-2">Emergency Contact Number</label>
                        {editMode ? (
                          <input
                            type="text"
                            value={editPersonalData.emergency_contact_phone || ''}
                            onChange={(e) => updatePersonalField('emergency_contact_phone', e.target.value)}
                            className="w-full px-3 md:px-4 py-2.5 md:py-3 bg-[#161616] border border-[#404040] rounded-lg focus:ring-2 focus:ring-[#2563eb] focus:border-transparent text-[#ffffff] font-light text-sm md:text-base"
                          />
                        ) : (
                          <p className="text-sm md:text-lg font-light text-[#ffffff] bg-[#111111] px-3 md:px-4 py-2.5 md:py-3 rounded-lg border border-[#2a2a2a]">
                            {personalData?.emergency_contact_phone || 'Not specified'}
                          </p>
                        )}
                      </div>
                ...
                <div className="bg-[#1e1e1e] rounded-xl border border-[#2a2a2a] p-4 md:p-6 hover-lift backdrop-blur-lg">
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                      ...
                      <div>
                        <label className="block text-xs md:text-sm font-light text-[#a3a3a3] mb-1.5 md:mb-2">Work Schedule</label>
                        <div className="flex items-center space-x-2.5 md:space-x-3 bg-[#111111] px-3 md:px-4 py-2.5 md:py-3 rounded-lg border border-[#2a2a2a]">
                      ...
                      <div>
                        <label className="block text-xs md:text-sm font-light text-[#a3a3a3] mb-1.5 md:mb-2">Job Family</label>
                        <p className="text-sm md:text-lg font-light text-[#ffffff] bg-[#111111] px-3 md:px-4 py-2.5 md:py-3 rounded-lg border border-[#2a2a2a]">
                      ...
                      <div>
                        <label className="block text-xs md:text-sm font-light text-[#a3a3a3] mb-1.5 md:mb-2">Management Level</label>
                        <p className="text-sm md:text-lg font-light text-[#ffffff] bg-[#111111] px-3 md:px-4 py-2.5 md:py-3 rounded-lg border border-[#2a2a2a]">
                    ...
                    <div className="space-y-4 md:space-y-6">
                      <div className="p-4 md:p-6 bg-[#111111] rounded-lg border border-[#2a2a2a]">
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0">
                            <h4 className="text-base md:text-lg font-light text-[#ffffff] mb-1">Password</h4>
                            <p className="text-xs md:text-sm font-light text-[#a3a3a3]">
                      ...
                      <div className="p-4 md:p-6 bg-[#111111] rounded-lg border border-[#2a2a2a]">
                        <div className="flex items-center justify-between">
                          <div>
                            <h4 className="text-base md:text-lg font-light text-[#ffffff] mb-1">Account Status</h4>
                            <p className="text-xs md:text-sm font-light text-[#a3a3a3]">
                              Your account is currently active and secure
                            </p>
                          </div>
                          <div className="flex items-center space-x-2">
                            <div className="w-2.5 h-2.5 md:w-3 md:h-3 bg-[#22c55e] rounded-full"></div>
                            <span className="text-xs md:text-sm font-light text-[#22c55e]">Active</span>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <div className="space-y-4 md:space-y-6">
              {/* Profile Stats */}
              <div className="bg-[#1e1e1e] rounded-xl border border-[#2a2a2a] p-3 md:p-4 hover-lift backdrop-blur-lg">
                <div className="absolute inset-0 bg-gradient-to-br from-[#f59e0b]/5 to-transparent rounded-xl"></div>
                <div className="relative z-10">
                  <h4 className="text-xs md:text-sm font-light text-[#ffffff] mb-3 md:mb-4 flex items-center space-x-2">
                  ...
                  <div className="space-y-2.5 md:space-y-3">
                    <div className="flex justify-between items-center p-2 bg-[#111111] rounded border border-[#2a2a2a]">
                      ...
                      <span className="text-xs md:text-sm font-light text-[#ffffff] cosmic-glow">
                        {employeeData?.employee_id || 'N/A'}
                      </span>
                    </div>
                    <div className="flex justify-between items-center p-2 bg-[#111111] rounded border border-[#2a2a2a]">
                      <span className="text-xs md:text-sm font-light text-[#a3a3a3]">Status</span>
                      <span className="text-xs md:text-sm font-light text-[#22c55e] cosmic-glow">
                        {employeeData?.status || 'Active'}
                      </span>
                    </div>
                    <div className="flex justify-between items-center p-2 bg-[#111111] rounded border border-[#2a2a2a]">
                      <span className="text-xs md:text-sm font-light text-[#a3a3a3]">Type</span>
                      <span className="text-xs md:text-sm font-light text-[#2563eb] cosmic-glow">
          ...
          {/* (Optional) Announcements — untouched functionality */}
          {/* You can add a mobile-compact list here if you enable it later */}
        </div>
      </div>
    </div>
  );
